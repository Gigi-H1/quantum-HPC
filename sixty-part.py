...
def run_simulation_and_get_counts(qc, shots=1024, method='statevector'):
    simulator = AerSimulator(method=method)
    compiled_circuit = transpile(qc, simulator)
    result = simulator.run(compiled_circuit, shots=shots).result()  # Specify the number of shots
...
# Run the simulation
combined_counts = run_simulation_and_get_counts(combined_qc, shots=1024, method='statevector')  # Adjust shots if needed

# Path to the output CSV file
csv_file = 'quantum_results7.csv'

# Write the results to a CSV file
write_counts_to_csv(combined_counts, csv_file)

# Print the counts and visualize the results
