...
def run_qiskit_simulation(qc):
    ...
    transpiled_circuit_counts = transpile(qc_with_measurements, simulator)
    result_counts = simulator.run(transpiled_circuit_counts, shots=20000).result()
    counts = result_counts.get_counts(transpiled_circuit_counts)

    # Run the simulation to get statevector (remove measurements)
    qc_no_measure = qc.remove_final_measurements(inplace=False)
    transpiled_circuit_statevector = transpile(qc_no_measure, simulator)
    result_statevector = simulator.run(transpiled_circuit_statevector, method='statevector').result()
